...
public partial class myECdata_SZ_SearchByDate : SecurityCheck
{
        ...
        try
        {
            if (!IsPostBack)
            {
                /* [取得MenuID, MenuName]
                   ...
                   取得對應的Type Name
                */
                string typeName = fn_Menu.GetECData_RefType(Convert.ToInt32(Req_TypeID)) + " (每日)";
                ...
                switch (Req_TypeID)
                {
                    case "1":
                        //工具
                        isPass = fn_CheckAuth.Check(fn_Param.CurrentUser, "3744");
                        break;

                    default:
                        //科玩
                        isPass = fn_CheckAuth.Check(fn_Param.CurrentUser, "3749");
                        break;
                }
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        //----- 原始資料:取得所有資料 -----
        var query = _data.GetECDList_byDate(search, Req_Lang, Convert.ToInt32(Req_TypeID)
        ...
        if (query.Count() == 0)
        {
            ...
            //Clear
            CustomExtension.setCookie("HomeList_ECDay", "", -1);
        }
        else
        {
            ...
            //暫存頁面Url, 給其他頁使用
            CustomExtension.setCookie("HomeList_ECDay", Server.UrlEncode(reSetPage), 1);
    ...
    public string thisPage
    {
        get
        {
            return "{0}/Date".FormatThis(FuncPath());
        }
        set
        {
            _thisPage = value;
