...
public partial class myOpcsRemark_SalesOrderEdit : SecurityCheck
{
    //設定FTP連線參數
    ...
    private void LookupData()
    {
        ...
        //----- 原始資料:取得所有資料 -----
        var query = _data.GetOne_OrderRemk(search, Req_DBS, out ErrMsg);
        ...
        string _SO_FID = query.Rows[0]["SO_FID"].ToString();
        string _SO_SID = query.Rows[0]["SO_SID"].ToString();
        ...
        lt_SO_FID.Text = _SO_FID;
        lt_SO_SID.Text = _SO_SID;
        hf_SO_FID.Value = _SO_FID;
        hf_SO_SID.Value = _SO_SID;
    ...
    private void Add_Data()
    {
        ...
        {
            //----- 方法:新增資料 -----
            Int32 _id = _data.Check_OrderRemk(Req_FID, Req_SID, Req_DBS, out ErrMsg);
            ...
            //更新本頁Url
            string thisUrl = "{0}SalesOrderEdit.aspx?dbs={1}&id={2}".FormatThis(FuncPath(), Req_DBS, _id);
            //Redirect
    ...
    private void Edit_Data()
    {
        ...
        try
        {
            ...
            //----- 方法:修改資料 -----
            if (!_data.Update_OrderRemk(_id, _remk1, out ErrMsg))
            {
                this.ph_ErrMessage.Visible = true;
    ...
    private void LookupData_Files()
    {
        ...
        //----- 原始資料:取得所有資料 -----
        var query = _data.Get_OpcsRemkFiles(Req_DataID, "B", out ErrMsg);
    ...
    protected void lv_Attachment_ItemCommand(object sender, ListViewCommandEventArgs e)
    {
        if (e.Item.ItemType == ListViewItemType.DataItem)
        {
            ...
            try
            {
                ...
                string _fid = hf_SO_FID.Value;
                string _sid = hf_SO_SID.Value;
                ...
                {
                    //刪除檔案
                    string ftpFolder = UploadFolder + _fid + _sid;
    ...
    protected void btn_SaveDetail_Click(object sender, EventArgs e)
    {
        ...
        {
            int errCnt = 0;
            string ftpFolder = UploadFolder + _fid + _sid; //ftp資料夾(站台資料夾+guid)
        ...
        if (fileList.Count > 0)
        {
            ...
            try
            {
                ...
                //----- 方法:更新資料 -----
                if (false == _data.Create_OpcsRemkFiles(dataItems, Req_DataID, "B", out ErrMsg))
                {
    ...
    /// 取得傳遞參數 - 單別
    /// </summary>
    private string _Req_FID;
    public string Req_FID
    {
        get
        {
    ...
    public string thisPage
    {
        get
        {
            return "{0}SalesOrderEdit.aspx?dbs={1}&id={2}".FormatThis(FuncPath(), Req_DBS, Req_DataID);
        }
        set
        {
            _thisPage = value;
    ...
    public string Page_SearchUrl
    {
        get
        {
            string tempUrl = CustomExtension.getCookie("OrderRemk");

            return string.IsNullOrWhiteSpace(tempUrl) ? FuncPath() + "SalesOrderSearch.aspx" : Server.UrlDecode(tempUrl);
        }
        set
        {
