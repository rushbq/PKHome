...
public partial class myShipping_Edit : SecurityCheck
{
    ...
    private void LookupData()
    {
        ...
        //----- 原始資料:取得所有資料 -----
        var query = _data.GetShipFreightList(search, null, Req_DataType, out ErrMsg).Take(1).FirstOrDefault();
        ...
        string _shipCompName = "德邦快递";  //default value (table:Logistics)
        this.tb_ShipComp.Text = string.IsNullOrWhiteSpace(query.ShipCompName) ? _shipCompName : query.ShipCompName;

        int _shipComp = 31; //default value (table:Logistics)
    ...
    private void Add_Data()
    {
        ...
        var erpData = _data.GetShipFreightList(search, null, Req_DataType, out ErrMsg).Take(1).FirstOrDefault();
        ...
        {
            Response.Redirect("{0}/Edit/{1}?erpNo={2}&dt={3}".FormatThis(FuncPath(), erpData.Data_ID, Req_ErpNo, Req_DataType));
            return;
        }
        ...
        {
            //更新本頁Url
            string thisUrl = "{0}/Edit/{1}?erpNo={2}&dt={3}".FormatThis(FuncPath(), guid, Req_ErpNo, Req_DataType);
    ...
    protected void btn_SaveDetail_Click(object sender, EventArgs e)
    {
        ...
        if (!this.tb_Freight.Text.IsNumeric())
        {
            errTxt += "運費欄位請填寫半形數字\\n";
        }
        //alert
        ...
        var data = new ShipFreightDetail
        {
            ...
            FreightWay = ddl_FreightWay.SelectedValue,
            Freight = Convert.ToDouble(this.tb_Freight.Text)
        };
    ...
    public string thisPage
    {
        get
        {
            return "{0}/Edit/{1}?dt={3}&erpNo={2}".FormatThis(FuncPath(), Req_DataID, Req_ErpNo, Req_DataType);
        }
        set
        {
