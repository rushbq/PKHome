...
public partial class myShipping_SendList : SecurityCheck
{
    ...
    private void LookupDataList(int pageIndex)
    {
        ...
        //----- 原始資料:取得所有資料 -----
        var query = _data.GetShipFreightSendList(search, Req_DataType, out ErrMsg);
    ...
    protected void lbtn_SendMail_Click(object sender, EventArgs e)
    {
        ...
        if (Send_Email(mailList, "[{0}]发货资料明细 {1}".FormatThis(fn_Menu.GetECData_RefType(Convert.ToInt16(Req_DataType)), dtToday), out ErrMsg))
        {
    ...
    private DataTable dataList()
    {
        ...
        var sumPay1 = query.Select(o => o.Freight).Sum();
    ...
    public string filterUrl()
    {
        ...
        //固定條件:Page/TOP選單
        url.Append("{0}?tab={1}&dt={2}&page=1".FormatThis(FuncPath(), Req_Tab, Req_DataType));
    ...
    {
        return "{0}{1}/{2}/ShipFreightSend_CHN".FormatThis(
            fn_Param.WebUrl
            , Req_Lang
            , Req_RootID);
    }
    ...
    public string thisPage
    {
        get
        {
            return "{0}?dt={1}&tab={2}".FormatThis(
                FuncPath()
                , Req_DataType
                , Req_Tab);
        }
